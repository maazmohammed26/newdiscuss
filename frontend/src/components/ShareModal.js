...
function TwitterIcon() {
  return <svg viewBox="0 0 24 24" className="w-6 h-6" fill="#000"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>;
}
        ...
        <div className="grid grid-cols-5 gap-2 mt-3">
          {shareOptions.map((opt) => (
            <a
              ...
              className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border border-[#E2E8F0] ${opt.color} transition-all`}
            >
              {opt.icon}
          ...
          <button
            ...
            onClick={handleCopy}
            className="flex flex-col items-center gap-1.5 p-3 rounded-xl border border-[#E2E8F0] hover:bg-[#3B82F6]/10 transition-all"
          >
            {copied ? <Check className="w-6 h-6 text-[#10B981]" /> : <Copy className="w-6 h-6 text-[#3B82F6]" />}
            <span className="text-[10px] text-[#64748B] font-medium">{copied ? 'Copied!' : 'Copy'}</span>
          </button>
        </div>
